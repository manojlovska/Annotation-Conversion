...
min_cart_distance = 0.02
# if the longest upper line in a square is this times longer than the longest bottom one, pick it (percantage)
upper_line_threshold = 2.5


# first line is annotaded line
def get_interstection(first_line, second_line, width, height):
    # account for annnotation mistakes
    #   for x on the left
    if first_line[0][0] - edge_margin <= 0:
        first_line[0][0] = 0
    if first_line[1][0] - edge_margin <= 0:
        first_line[1][0] = 0
    #   for x on the right
    if first_line[0][0] + edge_margin >= width:
        first_line[0][0] = width - 1
    if first_line[1][0] + edge_margin >= width:
        first_line[1][0] = width - 1
    #   for y on the top
    if first_line[0][1] - edge_margin <= 0:
        first_line[0][1] = 0
    if first_line[1][1] - edge_margin <= 0:
        first_line[1][1] = 0
    #   for y on the bottomt
    if first_line[0][1] + edge_margin >= height:
        first_line[0][1] = width - 1
    if first_line[1][1] + edge_margin >= height:
...
def get_cartesian_intersection(line1, line2):
    L1 = cartesian_line(line1)
    L2 = cartesian_line(line2)
...
def get_touching_squares(point, width, height, square_size):
    x = int(point[0])
    y = int(point[1])
...
def to_cartesian_xy(square_indexes, point, square_size):
    x = point[0]
    y = point[1]
...
def add_intersected_point(squares, polyline, square_index, cartesian_point):
...
def get_longest_cond_line(intersections, top_to_bottom=False):
    ...
    for intersection in intersections:
        ...
        if len(interesting_intersections) > 1 and (abs(intersection[1] - first_intersection[1]) >= min_cart_distance
                                                   and abs(intersection[0] - first_intersection[0]) >= min_cart_distance):
            break
    ...
    for intersection1 in interesting_intersections:
        for intersection2 in interesting_intersections:
            if intersection1 == intersection2 or [intersection2, intersection1] in longest_line:
                continue
            if (line_lenght(longest_line) < line_lenght([intersection1, intersection2])):
...
class Converter:
    def __init__(self, annotation_path, width, height, square_size):
        self.height = height
        self.width = width
        ...
        self.annotation_file = etree.parse(annotation_path)
    ...
    def to_cartesian(self, polylines):
        ...
        for polyline_index, polyline in enumerate(polylines):
            ...
            for intersection in intersections:
                for square_index in get_touching_squares(intersection, self.width, self.height, self.square_size):
                    add_intersected_point(intersected_squares, polyline_index, square_index, to_cartesian_xy(
        ...
        for i in range(0, len(intersected_squares)):
            ...
            for key in list(intersected_squares[i].keys()):
                ...
                if line_lenght(longest_line_tb) / line_lenght(longest_line_bt) > upper_line_threshold:
        ...
        for polyline_index, polyline in enumerate(polylines):
            for point in [polyline[0], polyline[len(polyline) - 1]]:
                touching_squares = []
                for square in get_touching_squares(point, self.width, self.height, self.square_size):
                ...
                cartesian_poly_point = to_cartesian_xy(
                ...
                for edge in cartesian_edges:
                    edge_intersection = get_cartesian_intersection(
                        [cartesian_poly_point, established_point], edge)
                    if edge_intersection and line_lenght([established_point, edge_intersection]) > longest_line_length:
                        longest_line_length = line_lenght(
                            [established_point, edge_intersection])
                        longest_line = [established_point, edge_intersection]

                if longest_line_length and line_lenght([established_point, cartesian_poly_point]) / longest_line_length >= extrapolation_threshhold:
            ...
            for key in list(intersected_squares[i].keys()):
                for j in range(0, len(intersected_squares)):
                    if i == j or (not intersected_squares[j]) or not key in intersected_squares[j]:
                        continue
                    if line_lenght(intersected_squares[i][key]) <= line_lenght(intersected_squares[j][key]):
                        del intersected_squares[i][key]
        ...
        return squares_output

    # get and scale all polylines for an image
    def get_polylines(self, image_name):
        path_expression = "//image[contains(@name,'" + image_name + "')]"
        # TODO exception if none or more images found
        annotated_image = self.annotation_file.xpath(path_expression)[0]
